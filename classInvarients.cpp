#include<iostream>

using namespace std;

//class invarient of a parent class Object should not be broken by child clss Object

class BankAccount{
    protected:
    double balance;
   public:
   BankAccount(double b){
    if(b<0) throw invalid_argument("Balance can't be negative");
    balance=b;
   }
   virtual void withdraw(double amount){
    if(balance - amount<0) throw runtime_error("insufficient funds");
    balance-=amount;
    cout<<"Amount withdraw.Remaining balance is"<<balance<<endl;
   }
};

//Brakes invarient: Should not be allowed.
class CheatAccount : public BankAccount {
    public:
     CheatAccount(double b):BankAccount (b){}

     void withdraw(double amount) override{
        balance-=amount;
        cout<<"Amount withdraw,Remaning balance is"<<balance<<endl;
     }

};

int main(){
    BankAccount* bankAccount = new BankAccount(100);
    bankAccount->withdraw(100);
}